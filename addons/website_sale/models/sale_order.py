...
class sale_order_line(osv.Model):
    ...
    def _fnct_get_discounted_price(self, cr, uid, ids, field_name, args, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            res[line.id] = line.tax_id.compute_all(res[line.id], 1, inverce=False)['total_included']
        return res
...
class website(orm.Model):
    ...
    _columns = {
        'pricelist_id': fields.related('user_id','partner_id','property_product_pricelist',
            type='many2one', relation='product.pricelist', string='Default Pricelist', store=False),
        #'pricelist_id': fields.related('default_lang_id','property_product_pricelist',
        #    type='many2one', relation='product.pricelist', string='Default Pricelist'),
        'currency_id': fields.related('pricelist_id','currency_id',
            type='many2one', relation='res.currency', string='Default Currency', store=False),
    }

    @ormcache(skiparg=3)
    def _get_pricelist(self, cr, uid, id, lang, context=None):
        website = self.browse(cr, SUPERUSER_ID, id)
        pricelist = False
        for lg in website.language_ids:
            if lg.code.split('_')[0] == lang.split('_')[0]:
                if website.user_id.login == 'public':
                    #pricelist = lg.property_product_pricelist
                    website.pricelist_id = lg.property_product_pricelist
                    #website.currency_id = lg.currency_id
                else:
                    website.pricelist_id = website.partner_id.property_product_pricelist
                    #website.currency_id = website.partner_id.property_product_pricelist.currency_id
                pricelist = website.pricelist_id
                break
        _logger.info("Price list %s:%s:%s:%s" % (pricelist, lang, lg, website.user_id.login))
        return pricelist

    def get_pricelist(self, cr, uid, ids, lang, context=None):
        return self._get_pricelist(cr, uid, ids[0], lang, context=context)
    ...
    def sale_get_order(self, cr, uid, ids, force_create=False, code=None, update_pricelist=None, context=None):
        ...
        if sale_order_id:
            ...
            pricelist_id = request.session.get('sale_order_code_pricelist_id') or request.website.pricelist_id.id if request.website.user_id.login == 'public' else partner.property_product_pricelist.id
            _logger.info("Sale oreder %s->%s=>%s:%s" % (request.website.user_id.login, pricelist_id, partner, request.website.partner_id))

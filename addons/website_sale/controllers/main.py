...
def get_pricelist():
    ...
    if sale_order:
        ...
        pricelist = request.website.get_pricelist(request.httprequest.cookies.get('website_lang'), context=context)
    if not pricelist:
...
class website_sale(http.Controller):
    ...
    def get_attribute_value_ids(self, product):
        ...
        website_currency_id = pool.get('product.price.type')._get_field_currency(cr, uid, 'list_price', context).id
        currency_id = self.get_pricelist().currency_id.id
        attribute_value_ids = []
        visible_attrs = set(l.attribute_id.id
                                for l in product.attribute_line_ids
                                    if len(l.value_ids) > 1)
        _logger.info("attributes %s:%s:%s:%s" % (request.website.pricelist_id.id, context['pricelist'], website_currency_id, currency_id))
        if request.website.pricelist_id.id != context['pricelist'] or website_currency_id != currency_id:
            #website_currency_id = request.website.currency_id.id
            #website_currency_id = request.website.get_pricelist(cr, uid, ids, request.website.default_lang_id.code, context=context).currency_id.id

            for p in product.product_variant_ids:
                price_vprice = currency_obj.compute(cr, uid, website_currency_id, currency_id, p.list_vprice)
                price = currency_obj.compute(cr, uid, website_currency_id, currency_id, p.lst_price)
                attribute_value_ids.append([p.id, [v.id for v in p.attribute_value_ids if v.attribute_id.id in visible_attrs], p.price_lst, price, price_vprice])
        else:
            attribute_value_ids = [[p.id, [v.id for v in p.attribute_value_ids if v.attribute_id.id in visible_attrs], p.price_lst, p.lst_price, p.list_vprice]
                for p in product.product_variant_ids]
        _logger.info("prices %s" % attribute_value_ids)
    ...
    def _get_unit_price(self, product_ids, add_qty, use_order_pricelist, vat=False):
        cr, uid, context, pool = request.cr, request.uid, request.context, request.registry
        ...
        if use_order_pricelist:
            pricelist_id = request.session.get('sale_order_code_pricelist_id') or self.get_pricelist().id
        else:
        ...
        if vat:
            ret = {product_id: [prices[product_id][pricelist_id][0], pool['product.product'].browse(cr, uid, product_id, context=context).product_tmpl_id.taxes_id.compute_all(prices[product_id][pricelist_id][0], 1, inverce=False)['total_included']] for product_id in product_ids}
        else:
            ret = {product_id: prices[product_id][pricelist_id][0] for product_id in product_ids}
        _logger.info("Get price %s" % ret)
        return ret
    ...
    def get_unit_price(self, product_ids, add_qty, use_order_pricelist=False, **kw):
        return self._get_unit_price(product_ids, add_qty, use_order_pricelist)

    @http.route(['/shop/get_unit_vat_price'], type='json', auth="public", methods=['POST'], website=True)
    def get_unit_price(self, product_ids, add_qty, use_order_pricelist=False, **kw):
        return self._get_unit_price(product_ids, add_qty, use_order_pricelist, True)
...
class Website(Website):
    ...
    def change_lang(self, lang, r='/', **kwargs):
        if lang == 'default':
            lg = request.website.default_lang_code
        else:
            lg = lang
        context = request.context
        pricelist = request.website.get_pricelist(lg, context=context)
        if pricelist:
            context['pricelist'] = int(pricelist.id)
        return super(Website, self).change_lang(lang, r=r, **kwargs)

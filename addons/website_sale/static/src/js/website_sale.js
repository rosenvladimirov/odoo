...
$('.oe_website_sale').each(function () {
    ...
    $(oe_website_sale).on("change", 'input[name="add_qty"]', function (event) {
        ...
        openerp.jsonRpc("/shop/get_unit_vat_price", 'call', {'product_ids': product_ids,'add_qty': parseInt(qty)})
        .then(function (data) {
            var current = product_dom.data("attribute_value_ids");
            for(var j=0; j < current.length; j++){
                current[j][3] = data[current[j][0]][1];
                current[j][4] = data[current[j][0]][0];
            }
            //console.log('get unit price', data)
    ...
    $(oe_website_sale).on('click', 'a.js_add_cart_json', function (ev) {
        ...
        $input.change();
        console.log('Add or remove qty', $input)
        return false;
    });
    ...
    $('form.js_add_cart_json label', oe_website_sale).on('mouseup touchend', function (ev) {
        ...
        console.log('Add cart form', $price)
    });
    ...
    $('.css_attribute_image input', oe_website_sale).on('change', function (ev) {
    ...
    $(oe_website_sale).on('change', 'input.js_product_change', function (ev) {
        ...
        console.log('Update product', $parent)
    });
    ...
    $(oe_website_sale).on('change', 'input.js_variant_change, select.js_variant_change, ul[data-attribute_value_ids]', function (ev) {
        ...
        var $price_vat = $parent.find(".oe_price_vat:first .oe_currency_value");
        ...
        //console.log('Update all', variant_ids)
        var product_id = false;
        for (var k in variant_ids) {
            //console.log('Show price', _.difference(variant_ids[k][1], values), values, variant_ids[k][1], variant_ids[k][2], variant_ids[k][3])
            if (_.isEmpty(_.difference(variant_ids[k][1], values))) {
                openerp.website.ready().then(function() {
                    $default_price.html(price_to_str(variant_ids[k][2]));
                    $price.html(price_to_str(variant_ids[k][3]));
                    $price_vat.html(price_to_str(variant_ids[k][4]));
                });

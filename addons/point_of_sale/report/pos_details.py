...
class pos_details(report_sxw.rml_parse):
    ...
    def _get_sum_invoice_2(self, form):
        ...
        company_id = user_obj.browse(self.cr, self.uid, self.uid).company_id.id
        date_start, date_end = self._get_utc_time_range(form)
        pos_ids = pos_obj.search(self.cr, self.uid, [('date_order','>=', date_start),('date_order','<=',date_end),('user_id','in',user_ids),('company_id','=',company_id),('invoice_ids','<>',False)])

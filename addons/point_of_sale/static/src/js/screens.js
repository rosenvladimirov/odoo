    ...
    module.ReceiptScreenWidget = module.ScreenWidget.extend({
        ...
        refresh: function() {
            ...
            $('.pos-receipt-container', this.$el).html(QWeb.render('PosTicket',{
                    widget:this,
                    order: order,
                    ean_id_image: order.get_ean_image(),
    ...
    */
    module.OrderListScreenWidget = module.ScreenWidget.extend({
        ...
        load_order_fields: function(order, fields, options) {
            ...
            order.setName(fields.name + ((options.mode === 'refund') ? ' REFUND' : ''));
            return order;
        },

        prepare_orderline_options: function(orderline, options){
            ...
            .then(function (result) {
                ...
                for (var i=0, len=orderlines.length; i<len; i++) {
                    ...
                    order.addProduct(product,
                                 self.prepare_orderline_options(orderline, options));
        ...
        load_order: function(order_id, order, mode) {
            var order_ids = [];
            var merge_mod = false;
            var fLen = 0;
            ...
            for (i = 0; i < fLen; i++) {
                this.load_order_db(order_ids[i], order, {mode: mode, merge: mode === 'merge' ? true : false});
                if(this.break_add){
                    break;
                } else {
        ...
        on_click_merge_order: function(event){
            ...
            if(order_ids.length > 0) {
                ...
                order = this.load_order(order_ids, order, 'merge');
        ...
        on_click_draft_order: function(event){
            ...
            var order = this.pos.get('selectedOrder');
            var order_id = parseInt(event.target.parentNode.dataset.orderId);
            order.get('orderLines').reset();
            order = this.load_order(order_id, order, 'standart');
            if(this.break_add !== true){
        ...
        on_click_refund_order: function(event){
            ...
            console.log("Orders selected", order_id, _.isEmpty(order_id));
            if(order_id) {
                order = this.reset_order(order);
                order = this.load_order(order_id, order, 'refund');
                if(this.break_add !== true){
        ...
        on_click_plus_order: function(event){
            ...
            console.log("Orders selected", order_ids);
            if(order_ids.length > 0) {
                order = this.load_order(order_ids, order, 'plus');
                if(this.break_add !== true){
                    this.$el.find('.button.next').show();
                }
            }
        ...
        on_click_next_order: function(event){
            ...
            if(current_order.get_order_state() === 'refund'){
                var totalWithoutTaxes = current_order.getTotalTaxExcluded();
                var tax = current_order.getTax();
                var total = current_order.getTotalTaxIncluded();
                current_order.get('orderLines').reset();
                this.pos_widget.order_widget.change_selected_order();
                console.log("Next", current_order.get_product_filter(), this.pos.pos_widget.product_filtered_screen, current_order);
                ...
                this.pos.pos_widget.product_filtered_screen.product_list_widget.set_custom_header(current_order.getName(),
                                                 this.pos.pos_widget.format_currency(totalWithoutTaxes, 'Product Price') + '/' +
                                                 this.pos.pos_widget.format_currency(tax, 'Product Price') + '/' +
                                                 this.pos.pos_widget.format_currency(total, 'Product Price'));
    ...
    module.PaymentScreenWidget = module.ScreenWidget.extend({
        ...
        validate_order: function(options) {
            ...
            if(currentOrder.getLoan() !== 0 && !currentOrder.get_client()){
            ...
            if(currentOrder.getLoan() > 0.00001){
                //var confirm = this.pos_widget.screen_messages.confirms('loan');
                //console.log("Loan is now" , confirm);
                //return confirm;
                var ret = this.pos_widget.screen_messages.confirms('loan');
                    if(ret){
                        if(currentOrder.getPaidAdvance() === 0){
                            this.pos_widget.screen_messages.errors('no-advance-payment');
                            return;
                        }
                        console.log("Loan", ret);
                        ret = this.pos_widget.screen_messages.confirms('confirm');
                        return;
                        if(ret){

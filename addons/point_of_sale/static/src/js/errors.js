...
function openerp_pos_errors(instance, module){ //module is instance.point_of_sale
    ...
    module.ErrorsWidget = module.PosBaseWidget.extend({
        ...
        confirms: function(confirm){
            var self = this;
            var ret = false;
            //console.log("Get confirm", confirm);
            if(typeof confirm === 'object'){
                this.pos_widget.screen_selector.show_popup(confirm['type'], confirm['content']);
                return confirm['content'];
            } else if(typeof confirm === 'string') {
                if (confirm === 'loan') {
                    console.log("error to loan", confirm);
                    ret = { type: 'confirm',
                            content: {
                                ret: false,
                                ...
                                confirm: function(){
                                    console.log("loan ok", confirm);
                                    this.ret = true;
                                    },
                                cancel: function(){
                                    console.log("loan cancel", confirm);
                                    this.ret = false;
                                    },
                                },
                            }
                } else if (confirm === 'deposit') {
                    console.log("error to deposit", confirm);
                    ret = { type: 'confirm',
                            content: {
                                ret: false,
                ...
                if(ret !== false) {
                    this.pos_widget.screen_selector.show_popup(ret['type'], ret['content']);

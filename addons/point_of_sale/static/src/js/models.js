function openerp_pos_models(instance, module){ //module is instance.point_of_sale
    ...
    module.PosModel = Backbone.Model.extend({
        ...
        models: [
        {
            ...
            domain: ['|',['customer','=',true],'&',['customer','=',false],['supplier','=',false]],
    ...
    module.Order = Backbone.Model.extend({
        initialize: function(attributes){
            ...
            this.ean_uid = this.generate_ean_id();
            this.set({
                ...
                state:          'new',
                product_filter: [],
                pricelist_id: this.pos.config.pricelist_id[0], // default price list
                fiscal_position: this.pos.db.get_partner_by_id(this.pos.company.partner_id[0]).property_account_position[0], //default fiscal position
                });
        ...
        generateUniqueId: function(options) {
            var separator = _.isEmpty(options) ? '-' : options.separator;
            function zero_pad(num,size){
                return ("0".repeat(size)+num.toString()).slice(-size);
            }
            return zero_pad(this.pos.pos_session.id,5) +separator+
                   zero_pad(this.pos.pos_session.login_number,3) +separator+
                   zero_pad(this.sequence_number,4);
        },
        generate_ean_id: function(){
            return this.pos.barcode_reader.sanitize_ean('43'+this.generateUniqueId({'separator': ''}).slice(-10));
        },
        ...
        set_ean_id: function(ean) {
            this.ean_uid = ean;
        },
        get_ean_image: function() {
            return window.location.origin + '/report/barcode/?type=EAN13&value='+this.ean_uid+'&width=300&height=100&humanreadable=true';
        },
        ...
        getChange: function() {
            var change = this.getPaidTotal(['money']) - this.getTotalTaxIncluded();
            if (change < 0.0){
                change = 0.0;
            }
            return change;
        ...
        getLoan: function(){
            var loan = this.getTotalTaxIncluded() - this.getPaidTotal(['money', 'advance']);
            if (loan < 0.0){
                loan = 0.0;
            }
            return Math.abs(loan);
        },
        getDueLeft: function() {
        ...
        set_client: function(client){
            ...
            var fp_id = false;
            console.log("Company", this.pos.company, this.pos.db.partner_by_id, this.pos.company.partner_id[0], this.pos.db.get_partner_by_id(this.pos.company.partner_id[0]));
            if (client && client.property_product_pricelist) {
                price_list_id = client.property_product_pricelist[0];
            } else {
                price_list_id = this.pos.config.pricelist_id[0];
            }
            if (client && client.property_account_position) {
                fp_id = client.property_account_position;
            } else {
                fp_id = this.pos.db.get_partner_by_id(this.pos.company.partner_id[0]).property_account_position[0];
            }
            this.set('fiscal_position', fp_id);
            this.set('pricelist_id', price_list_id);
            this.set('client',client);
        },
        get_pricelist_id: function(){
            return this.get('pricelist_id') || this.pos.config.pricelist_id[0];
        },
        get_fp: function(){
            return this.get('fiscal_position');
        },
        get_client: function(){
        ...
        export_as_JSON: function() {
            ...
            console.log("order id", this.get_order_id());
            return {
                name: this.getName(),
                ...
                pricelist_id: this.get_pricelist_id() ? parseInt(this.get_pricelist_id()) : false,
                fiscal_position: this.get_fp() ? parseInt(this.get_fp()) : false,
            ...
            _(taxes).each(function(tax) {
                if (tax.price_include || tax_include) {
                    if (tax.type === "percent") {
                        ...
                        data = {base: base - tmp, amount:tmp, price_include:true, id: tax.id};
                        res.push(data);
                    } else if (tax.type === "fixed") {
                        tmp = tax.amount * self.get_quantity();
                        data = {base: base, amount:tmp, price_include:true, id: tax.id};
                        res.push(data);
                    } else {
                    ...
                    if (tax.type === "percent") {
                        ...
                        data = {base: base, amount:tmp, price_include:false, id: tax.id};
                        res.push(data);
                    } else if (tax.type === "fixed") {
                        tmp = tax.amount * self.get_quantity();
                        data = {base: base, amount:tmp, price_include:false, id: tax.id};

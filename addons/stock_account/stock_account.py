    ...
    _columns = {
        ...
        'valuation_out_account_id': fields.many2one('account.account', 'Stock Valuation Account (Outgoing)', domain=[('type', '=', 'other')],
                                                   help="Used for real-time inventory valuation. When set on a virtual location (non internal type), "
                                                        ...
                                                        "This has no effect for internal locations."),
        'in_tax_id': fields.many2one('account.tax', 'Fiscal position (Incoming)', domain=[('type_tax_use', '=', 'sale')],
                                                   help="Used for real-time inventory valuation. When set on a virtual location (non internal type), "
                                                        "this fiscal position will be used to calculate taxes for the tax accrued in writing inventory shortages during inventory. "
                                                        "Primarily used for wasting scrap or missing. "
                                                        "This has no effect for internal locations."),
    }
...
class stock_quant(osv.osv):
    ...
    def _account_entry_move(self, cr, uid, quants, move, context=None):
        ...
        parts = {'location_from': location_from, 'location_to': location_to, 'company_from': company_from, 'company_to': company_to, 'quants': quants, 'move': move}
        ...
        parts = self._account_entry_move_ex(cr, uid, parts, context=context)
        if company_to and (move.location_id.usage not in ('internal', 'transit') and move.location_dest_id.usage == 'internal' or company_from != company_to):
            ...
            journal_id, acc_src, acc_dest, acc_valuation, tax_id = self._get_accounting_data_for_valuation(cr, uid, move, context=ctx)
            _logger.info("Ussages first %s:%s->%s:%s:%s:%s" % (move.location_id.usage, move.location_dest_id.usage, tax_id, acc_src, acc_dest, acc_valuation))
            if location_from and location_from.usage == 'customer':
                #goods returned from customer
                self._create_account_move_line(cr, uid, quants, move, acc_dest, acc_valuation, False, journal_id, context=ctx)
            else:
                self._create_account_move_line(cr, uid, quants, move, acc_src, acc_valuation, False, journal_id, context=ctx)
        ...
        if company_from and (move.location_id.usage == 'internal' and move.location_dest_id.usage not in ('internal', 'transit') or company_from != company_to):
            ...
            journal_id, acc_src, acc_dest, acc_valuation, tax_id = self._get_accounting_data_for_valuation(cr, uid, move, context=ctx)
            _logger.info("Ussages second %s:%s->%s:%s:%s:%s" % (move.location_id.usage, move.location_dest_id.usage, tax_id, acc_src, acc_dest, acc_valuation))
            if location_to and location_to.usage == 'supplier':
                #goods returned to supplier
                self._create_account_move_line(cr, uid, quants, move, acc_valuation, acc_src, False, journal_id, context=ctx)
            else:
                self._create_account_move_line(cr, uid, quants, move, acc_valuation, acc_dest, tax_id, journal_id, context=ctx)
    ...
    def _get_accounting_data_for_valuation(self, cr, uid, move, context=None):
        ...
        if move.location_dest_id.in_tax_id:
            tax_id = tax_obj.browse(cr, uid, move.location_dest_id.in_tax_id.id)

        acc_valuation = accounts.get('property_stock_valuation_account_id', False)
        journal_id = accounts['stock_journal']
        return journal_id, acc_src, acc_dest, acc_valuation, tax_id

    def _prepare_account_move_line(self, cr, uid, move, qty, cost, credit_account_id, debit_account_id, tax_id, context=None):
        """
...
class stock_move(osv.osv):
    ...
    def action_done(self, cr, uid, ids, context=None):
        #_logger.debug("Stock move action %s" % ids)
    ...
    def product_price_update_after_done(self, cr, uid, ids, context=None):
        ...
        '''
        for move in self.browse(cr, uid, ids, context=context):
            _logger.debug("Product price update after done: %s:%s" % (move.product_id.cost_method, move.location_dest_id.usage))
